...
class AdminDashboard(QWidget):
    ...
    def initUI(self):
        ...
        self.employees_table = QTableWidget(0, 3)
        self.employees_table.setStyleSheet("background-color: white; color: black;")
        self.employees_table.setFixedSize(600, 200)

        self.patients_table.setHorizontalHeaderLabels(['Фамилия', 'Имя', 'Номер палаты', 'Время записи на прием', 'Врач'])
        self.employees_table.setHorizontalHeaderLabels(['Фамилия', 'Имя', 'Специальность'])
        ...
        add_employees_button.setStyleSheet("background-color: white; color: black;")
        edit_employees_button.clicked.connect(self.edit_employee)
        edit_employees_button.setStyleSheet("background-color: white; color: black;")
        delete_employees_button.clicked.connect(self.delete_employee)
        delete_employees_button.setStyleSheet("background-color: white; color: black;")

        patients_layout = QHBoxLayout()
        employees_layout = QHBoxLayout()

        patients_buttons_layout = QVBoxLayout()
        employees_buttons_layout = QVBoxLayout()
        ...
        employees_buttons_layout.addWidget(add_employees_button)
        employees_buttons_layout.addWidget(edit_employees_button)
        employees_buttons_layout.addWidget(delete_employees_button)
    ...
    def load_patient_data(self):
        ...
        for row, patient in enumerate(patients):
            self.patients_table.setItem(row, 0, QTableWidgetItem(str(patient.patient_name)))
            self.patients_table.setItem(row, 1, QTableWidgetItem(str(patient.patient_last_name)))
            self.patients_table.setItem(row, 2, QTableWidgetItem(str(patient.patient_phone_number)))
            self.patients_table.setItem(row, 3, QTableWidgetItem(str(patient.patient_reception_time)))
            self.patients_table.setItem(row, 4, QTableWidgetItem(str(patient.employee_id)))
    ...
    def load_employee_data(self):
        ...
        for row, employee in enumerate(employees):
            self.employees_table.setItem(row, 0, QTableWidgetItem(str(employee.employee_name)))
            self.employees_table.setItem(row, 1, QTableWidgetItem(str(employee.employee_last_name)))
            self.employees_table.setItem(row, 2, QTableWidgetItem(str(employee.employee_specialty)))
