...
from PyQt6.QtWidgets import QLabel, QLineEdit, QPushButton, QVBoxLayout, QMessageBox, QWidget, QTableWidget, \
    QHBoxLayout, QTableWidgetItem, QApplication, QFormLayout, QComboBox, QTimeEdit, QDateEdit
...
class AdminDashboard(QWidget):
    ...
    def initUI(self):
        ...
        self.patients_table = QTableWidget(0, 7)
        ...
        self.patients_table.setHorizontalHeaderLabels(
            ['ID', 'Фамилия', 'Имя', 'Номер палаты', 'Дата записи', 'Время приема', 'Врач'])
    ...
    def load_patient_data(self):
        ...
        for row, patient in enumerate(patients):
            ...
            self.patients_table.setItem(row, 4, QTableWidgetItem(str(patient.patient_reception_date)))
            self.patients_table.setItem(row, 5, QTableWidgetItem(str(patient.patient_reception_time)))
            self.patients_table.setItem(row, 6, QTableWidgetItem(str(patient.employee_id)))
    ...
    def add_patient(self):
        ...
        reception_date_input = QDateEdit()
        reception_date_input.setDisplayFormat("dd-MM-yyyy")

        reception_time_input = QTimeEdit()
        reception_time_input.setDisplayFormat("hh:mm")

        employee_id_combo = QComboBox()
        self.load_employee_ids(employee_id_combo)

        layout.addRow("Фамилия:", last_name_input)
        layout.addRow("Имя:", name_input)
        layout.addRow("Номер телефона:", phone_number_input)
        layout.addRow("Дата записи на прием:", reception_date_input)
        layout.addRow("Время записи на прием:", reception_time_input)
        layout.addRow("Лечащий врач:", employee_id_combo)

        add_button = QPushButton("Добавить")
        add_button.clicked.connect(lambda: self.save_new_patient(
            name_input.text(), last_name_input.text(), phone_number_input.text(),
            reception_date_input.date().toString("dd-MM-yyyy"),
            reception_time_input.time().toString("HH:mm"),
            employee_id_combo.currentData()))
        layout.addRow(add_button)

        self.patient_form_window.setLayout(layout)
        self.patient_form_window.show()

    def load_employee_ids(self, combo_box):
        db = SessionLocal()
        ...
        combo_box.clear()
        combo_box.addItem("Выберите врача")

        for employee in employees:
            combo_box.addItem(f"{employee.employee_id} - {employee.employee_last_name} {employee.employee_name}",
                              userData=employee.employee_id)

    def save_new_patient(self, name, last_name, phone_number, reception_date, reception_time, employee_id):
        db = SessionLocal()
        patients_service = PatientsService(db)
        try:
            patients_service.add_patients(name, last_name, phone_number,  reception_date, reception_time, employee_id)
            QMessageBox.information(self, 'Успех', 'Пациент добавлен успешно!')
            self.load_patient_data()
        except Exception as e:
            QMessageBox.warning(self, 'Ошибка', f'Ошибка при добавлении пациента: {str(e)}')
        finally:
            db.close()
    ...
    def edit_patient(self):
        ...
        if selected_row >= 0:
            ...
            name_input = QLineEdit(self.patients_table.item(selected_row, 1).text())
            last_name_input = QLineEdit(self.patients_table.item(selected_row, 2).text())
            phone_number = QLineEdit(self.patients_table.item(selected_row, 3).text())

            reception_date_str = self.patients_table.item(selected_row, 4).text()
            reception_date = QDateEdit(self.patient_form_window)
            reception_date.setDate(QDate.fromString(reception_date_str, "dd-MM-yyyy"))

            reception_time = QTimeEdit(self.patient_form_window)
            reception_time.setTime(QTime.fromString(self.patients_table.item(selected_row, 5).text(), "HH:mm"))

            employee_id_combo = QComboBox()
            self.load_employee_ids(employee_id_combo)

            current_employee_id = int(self.patients_table.item(selected_row, 6).text())
            employee_id_combo.setCurrentIndex(employee_id_combo.findData(current_employee_id))

            layout.addRow("Фамилия:", last_name_input)
            layout.addRow("Имя:", name_input)
            layout.addRow("Номер телефона:", phone_number)
            layout.addRow("Дата записи на прием:", reception_date)
            ...
            edit_button.clicked.connect(lambda: self.save_edits_patient(
                patient_id, last_name_input.text(), name_input.text(), phone_number.text(),
                reception_date.date().toString("dd-MM-yyyy"), reception_time.time().toString("HH:mm"),
                employee_id_combo.currentData()))
    ...
    def save_edits_patient(self, patient_id, name, last_name, phone_number, reception_date, reception_time, employee_id):
        db = SessionLocal()
        patients_service = PatientsService(db)
        try:
            patients_service.update_patient(patient_id, name, last_name, phone_number, reception_date, reception_time, employee_id)
